...
const Starfield = ({ totalValue }: { totalValue: number }) => {
  const meshRef = useRef<THREE.Points>(null);
  const count = 500;

  const particles = React.useMemo(() => {
    return new Float32Array(1500).map((_, i) => (Math.sin(i * (totalValue || 123.456)) * 100));
  }, [totalValue]);
...
<Canvas camera={{ position: [0, 15, 20], fov: 45 }}>
        ...
        <Starfield totalValue={totalValue} />
...
export default function OrbitalPortfolio() {
  ...
  return (
    <div className="card glass-panel interactive-element" style={{ height: '400px', width: '100%', position: 'relative', padding: 0, overflow: 'hidden', borderLeft: '4px solid #8b5cf6' }}>
      ...
      {/* 3D Scene */}
      <CanvasScene portfolio={portfolio} totalValue={totalValue} onSelectAsset={onSelectAsset} activeSymbolHook={activeSymbolHook} />
    </div>
  );
}
