...
async def fetch_historical_data(ticker: str, timeframe: str = "1d", limit: int = 100):
    ...
    """
    logger.info(
        f"Fetching {limit} bars of {timeframe} historical data for {ticker} via Binance..."
    )
...
async def run_backtest(ticker: str = "BTC", timeframe: str = "1d", limit: int = 10):
    ...
    df = await fetch_historical_data(
        ticker, timeframe, limit=limit + 50
    )  # fetch extra for indicators
...
def generate_backtest_report(backtest_data: dict):
    ...
    md_content = "# Agentic Backtesting Report\\n\\n"
    ...
    md_content += "## 1. Summary of Actions\\n\\n"
    md_content += (
        f"```json\\n{json.dumps(backtest_data['summary'], indent=2)}\\n```\\n\\n"
    )

    md_content += "## 2. Detailed Event Log\\n\\n"
