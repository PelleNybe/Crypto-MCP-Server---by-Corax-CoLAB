...
mcp = FastMCP(
    name="llm", stateless_http=True, json_response=True, host="0.0.0.0", port=7015
)
...
try:
    ...
    logger.warning(
        "litellm is not installed. AI functionality will be simulated or limited. run `pip install litellm`"
    )
...
def generate_text(
    ...
    if not LITELLM_AVAILABLE:
        return {
            "error": "litellm library is not installed. Please install it to use open-source AI integration."
        }
...
if __name__ == "__main__":
    print(
        f"Starting llm_mcp on http://127.0.0.1:7015/mcp (Model: {LLM_MODEL}) — Crypto MCP Server (Corax CoLAB)"
    )
