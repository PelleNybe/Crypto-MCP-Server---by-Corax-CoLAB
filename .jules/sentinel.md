...
## 2026-04-24 - [Information Disclosure / DoS] Overly Permissive CORS Configuration
**Vulnerability:** The Express application (`gui/backend/server.js`) used a permissive CORS configuration `app.use(cors())` and `io({ cors: { origin: "*" } })`, allowing requests from any domain.
**Learning:** Permissive CORS allows any site to make cross-origin requests and read responses, potentially leading to unauthorized access to user-specific data or performing actions on behalf of the user.
**Prevention:** Always restrict CORS explicitly using `corsOptions` with an origin check, and ensure Socket.io instances follow the same restrictions.
